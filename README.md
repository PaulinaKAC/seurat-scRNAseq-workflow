...
| Plot | Description |
| *Violin QC plots* | Quality control such as `nFeature_RNA`, `percent.mt` before and after filtering. |
| *UMAP by dataset* | How well the integration mixes different datasets; checks for batch effects. |
| *UMAP after DoubletFinder* |Shows the amount of doublets in the sample.|
| *Cluster UMAPs* | Helps decide the right clustering resolution for your needs. |
